...
class SalesforceIntegrator:
    ...
    def get_activities_in_progress(self, owner_id: Optional[str] = None, limit: int = 50) -> Dict[str, List[Dict[str, Any]]]:
        """Retorna atividades em andamento:

        - Tasks cujo Status não seja 'Completed' (ou equivalente)
        - Events com EndDateTime >= TODAY (próximos/abertos)
        - Opportunities não fechadas (IsClosed = false)

        owner_id: opcional, filtrar por OwnerId (ID do usuário)
        limit: limite por cada tipo de registro
        """
        if not self.connected:
            if not self.connect():
                print("❌ Não conectado ao Salesforce; não é possível buscar atividades.")
                return {"tasks": [], "events": [], "opportunities": []}

        results = {"tasks": [], "events": [], "opportunities": []}

        try:
            # Tasks abertas
            where_task = "Status != 'Completed'"
            if owner_id:
                where_task += f" AND OwnerId = '{owner_id}'"
            query_tasks = f"SELECT Id, Subject, Status, Priority, OwnerId, WhatId, ActivityDate, CreatedDate FROM Task WHERE {where_task} ORDER BY ActivityDate NULLS LAST LIMIT {limit}"
            task_res = self.sf.query(query_tasks)
            if task_res and task_res.get('totalSize', 0) > 0:
                results['tasks'] = task_res['records']

            # Events futuros (EndDateTime >= TODAY) - para compatibilidade usamos EndDateTime se disponível
            where_event = "EndDateTime >= TODAY"
            if owner_id:
                where_event += f" AND OwnerId = '{owner_id}'"
            query_events = f"SELECT Id, Subject, StartDateTime, EndDateTime, OwnerId, WhoId, WhatId FROM Event WHERE {where_event} ORDER BY StartDateTime ASC LIMIT {limit}"
            try:
                event_res = self.sf.query(query_events)
                if event_res and event_res.get('totalSize', 0) > 0:
                    results['events'] = event_res['records']
            except Exception:
                # Alguns orgs usam campos diferentes/ versões; fallback para eventos sem filtro de data
                fallback_events = self.sf.query(f"SELECT Id, Subject, StartDateTime, EndDateTime, OwnerId FROM Event ORDER BY StartDateTime ASC LIMIT {limit}")
                if fallback_events and fallback_events.get('totalSize', 0) > 0:
                    results['events'] = fallback_events['records']

            # Opportunities em andamento (não fechadas)
            where_opp = "IsClosed = false"
            if owner_id:
                where_opp += f" AND OwnerId = '{owner_id}'"
            query_opps = f"SELECT Id, Name, StageName, Amount, CloseDate, OwnerId FROM Opportunity WHERE {where_opp} ORDER BY CloseDate ASC LIMIT {limit}"
            opp_res = self.sf.query(query_opps)
            if opp_res and opp_res.get('totalSize', 0) > 0:
                results['opportunities'] = opp_res['records']

            print(f"✅ Atividades obtidas: tasks={len(results['tasks'])}, events={len(results['events'])}, opportunities={len(results['opportunities'])}")
            return results
